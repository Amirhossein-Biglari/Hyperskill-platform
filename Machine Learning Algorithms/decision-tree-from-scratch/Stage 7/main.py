...
df_train = pd.read_csv('data_stage7.csv', index_col='Unnamed: 0')

# Initialize the decision tree with a root node
ds = DecisionTree(Node(), minimum_num_samples=1)

# Find the best split using the _split_function
minimum_weighted_gini, feature_name, split_value, left_indices, right_indices = \
    ds._split_function(df_train.iloc[:, :-1], df_train.iloc[:, -1])

# Print the best split results
print(round(minimum_weighted_gini, 3), feature_name, round(split_value, 3), left_indices, right_indices)
