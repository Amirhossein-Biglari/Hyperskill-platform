...
class DecisionTree:
    ...
    def _split_function(self, features: pd.DataFrame, targets: pd.Series):
        ...
        for column in features.columns:
            for column_value in features[column].unique():
                ...
                weighted_gini_index = self._calculate_weighted_gini_impurity(left_node_targets, right_node_targets)

                if weighted_gini_index < least_gini_index:
                    least_gini_index = weighted_gini_index
                    feature_name = column
    ...
    def _recursive_splitting(self, node, features: pd.DataFrame, targets: pd.Series):
        ...
        print(f'{round(weighted_gini_index, 3)} {feature_name} {split_value} {left_indices} {right_indices}')
...
train_address = r'test\data_stage7.csv'
df_train = pd.read_csv(input(), index_col=0)

root = Node()

decision_tree = DecisionTree(root, minimum=1)
feature_name, split_value, left_indices, right_indices, weighted_gini_index = \
    decision_tree._split_function(df_train.iloc[:, :-1], df_train.iloc[:, -1])
print(round(weighted_gini_index, 3), feature_name, round(split_value, 3), left_indices, right_indices)
